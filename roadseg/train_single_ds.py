...
def valid_one_epoch(model, dataloader, optimizer, device, epoch, criterion):
    ...
    val_scores = [val_scores[0], val_scores[1], (2*((val_scores[0] * val_scores[1]) / (val_scores[0] + val_scores[1])))]
    torch.cuda.empty_cache()
    gc.collect()
...
def run_training(model, model_name, optimizer, scheduler, train_loader, valid_loader, device, num_epochs, criterion, use_wandb, log_dir, plot_freq=3):
    ...
    for epoch in range(1, num_epochs + 1): 
        ...
        val_loss, val_scores, last_in, last_pred, last_msk = valid_one_epoch(model, valid_loader, optimizer, 
                                                                            criterion=criterion,
                                                                             device=device, 
                                                                             epoch=epoch)
        
        if plot_freq >= 1 and epoch % plot_freq == 1:
            plot_batch(last_in.cpu().numpy(), last_msk.cpu().numpy(), preds=last_pred.cpu().numpy(), src=f"val-epoch-{epoch}",log_dir=log_dir)
        if use_wandb: 
        ...
        if use_wandb:
            ...
            wandb.log({f'{model_name} Valid Precision': val_precision, 'Valid Recall': val_recall},step = global_step)
            wandb.log({f'{model_name} Epoch': epoch}, step = global_step)
        
        print(f'Valid Loss: {val_loss:0.4f} | Valid F1: {val_f1:0.4f} | Valid Precision: {val_precision:0.4f} | Valid Recall: {val_recall:0.4f}')
...
def pretrain_model(CFG, model, train_loader, val_loader, n_train_samples=None):
    ...
    model, history_pre = run_training(model, model_name, optimizer, scheduler, train_loader, val_loader, 
                                  criterion=BCELoss,
                                  device=CFG.device,
                                  use_wandb=CFG.wandb,
                                  log_dir=CFG.log_dir,
...
def evaluate_finetuning(pretrained_model, comp_splits, CFG, n_comp_samples=None):
    ...
    for fold, (train_loader, val_loader) in enumerate(comp_splits):
        ...
        optimizer = optim.Adam(model.parameters(), lr=CFG.finetuning_lr, weight_decay=CFG.weight_decay)
        scheduler = fetch_scheduler(optimizer, CFG=CFG, n_train_samples=n_comp_samples)
        model, history = run_training(model, model_name, optimizer, scheduler, train_loader, val_loader,
