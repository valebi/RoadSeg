...
def get_dataloaders(CFG, transforms):
    ...
    datasets =[dataset_map[ds](CFG, transforms=transforms) for ds in CFG.datasets]
    ...
    val_loader = DataLoader(val_dataset, batch_size=CFG.val_batch_size, 
                            num_workers=CFG.num_workers, shuffle=True, pin_memory=True, drop_last=False)
    ...
    comp_splits = []
    kf = KFold(n_splits=CFG.n_finetuning_folds, random_state=42, shuffle=True) # these folds should not change! Even if the rest runs nondeterministically
    for train_index, val_index in kf.split(np.arange(len(comp_dataset))):
        ...
        comp_train_loader = DataLoader(train_subset, batch_size=CFG.train_batch_size, 
                                  num_workers=CFG.num_workers, shuffle=True, pin_memory=True, drop_last=False)
        # val dataset is shuffled to get random plots
        comp_val_loader = DataLoader(val_subset, batch_size=CFG.val_batch_size, 
                                  num_workers=CFG.num_workers, shuffle=True, pin_memory=True, drop_last=False)
