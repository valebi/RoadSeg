...
from roadseg.inference_diffusion import generate_predictions, pred_from_dataloader
        ...
        if (step + 1) % n_accumulate == 0:
            ...
            if scheduler is not None:
                if not isinstance(scheduler, optim.lr_scheduler.ReduceLROnPlateau):
                    ...
                    if step == 0:
                        scheduler.step(loss)  ##Checks at every epoch
...
def valid_one_epoch(
    ...
    for step, (images, masks) in pbar:
        ...
        # y_pred = diffusion.sample(images).cpu().detach()
    ...
    return epoch_loss, val_scores, images, torch.zeros_like(labels), labels
...
def evaluate_finetuning(pretrained_model, comp_splits, CFG):
    ...
    return 0  # np.mean(scores_to_watch)
